    ...
    public class ProjectMV : Project
    {
        public static async Task<ReturnResult> SelectById(long id, long companyId)
        {
            try
            {
                ...
                ProjectMV oProjectMV = ObjectManipulation.CastObject<ProjectMV>(await db.Projects.FirstAsync(c => c.Id == id && c.CompanyId == companyId));
                return new ReturnResult(true, "", oProjectMV);
            }
            catch (Exception ex)
            {
        ...
        public static async Task<ReturnResult> SelectAll(long companyId)
        {
            ...
            {
                using QerpContext db = new QerpContext();
                List<ProjectMV> lstProjectMV = ObjectManipulation.CastObject<List<ProjectMV>>(await db.Projects.Where(x => x.CompanyId == companyId).ToListAsync());
                return new ReturnResult(true, "", lstProjectMV);
            }
            catch (Exception ex)
            {
                return new ReturnResult(false, ex.Message, null);
